...
class CLSTM(nn.Module):
    ...
    def __init__(self, shape, input_chans, filter_size, num_features,num_layers):
        ...
        cell_list=[]
        cell_list.append(CLSTM_cell(self.shape, self.input_chans, self.filter_size, self.num_features))#the first
    ...
    def forward(self, input):
        ...
        #print(len(self.hidden_state))
        final_outs=[]
        for idlayer in range(self.num_layers):#loop for every layer
            #print(idlayer)
            hidden_c=self.hidden_state[idlayer]#hidden and c are images with several channels
            all_output = []
            output_inner = []            
            for t in range(seq_len):#loop for every step
                ...
                #print(len(hidden_c))
                self.hidden_state[idlayer]=hidden_c#mychange
    ...
    def init_hidden(self,batch_size):
        ...
        for i in range(self.num_layers):
            init_states.append(self.cell_list[i].init_hidden(batch_size))
        self.hidden_state=init_states
    def init_hidden_known(self,values):
        self.hidden_state=values
        #print(len(self.hidden_state))
###########Usage#######################################    
"""num_features=10
filter_size=5
